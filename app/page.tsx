      ...
      <section className='grid grid-cols-5 w-full container gap-2 mt-16'>
        {countries.map((country) => (
          <Link key={country.name.common} href={`/pais/${country.name.common}`}>
            <article className='h-64 min-w-full p-2 bg-white border-2 rounded-xl  hover:border-gray-500 transition-all hover:shadow-xl' key={country.name.common}>
            <div className='relative w-full h-40 p2 overflow-hidden rounded-xl'>
              <Image className='object-cover' src={country.flags.svg} alt={country.flags.alt} fill/>
            </div>
